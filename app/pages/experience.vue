...
<template>
  <div class="grid grid-cols-1 lg:grid-cols-10 gap-6 items-start">
    ...
    >
      <UTimeline :items="experiences" class="w-full">
        ...
        <template #description="{ item }">
          ...
          <p v-if="item.team" class="text-xs text-dimmed mt-1">
    ...
    >
      <div class="space-y-6">
        <div>
          <h3 class="font-bold mb-3">{{ t("skills.technicalTitle") }}</h3>
          <div class="space-y-3">
            <div v-for="category in technicalSkills" :key="category.label">
              <p class="text-sm text-muted mb-1">{{ category.label }}</p>
        ...
        <div>
          <h3 class="font-bold mb-3">{{ t("skills.softTitle") }}</h3>
          <div class="space-y-3">
            <div v-for="category in softSkills" :key="category.label">
              <p class="text-sm text-muted mb-1">{{ category.label }}</p>
              <div class="flex flex-wrap gap-1">
                <UBadge
                  v-for="item in category.items"
